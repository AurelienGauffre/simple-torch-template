...
from datamodules import Cifar10DataModuleSwaV, Cifar10DataModuleSup,ImagenetteDataModuleSwaV, ImagenetteDataModuleSup,ImagenetteDataModuleSwavSup
...
parser.add_argument('--run_name', type=str, help='group name in wandb', default='MTL')
...
if __name__ == "__main__":
    ...
    model = MTLSwavSup(params)
    dm_MTL = ImagenetteDataModuleSwavSup(params,randaugment=RANDAUGMENT)

    checkpoint_callback = ModelCheckpoint(dirpath=params.root_dir, filename='SwaVMTL-{epoch}-{train_loss:.2f}',
                                          every_n_epochs=SAVE_EVERY_N_EPOCHS, save_top_k=20, monitor="train_loss")
    ...
    #
    #     model_loaded = MTLSwavSup.load_from_checkpoint(ckpt,params=params)
